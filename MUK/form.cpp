...
void Form::teclas()
{
    ...
    if(nivel != 7)if(Muk->x() > 600 && Muk->x() < 1540) scene->setSceneRect(Muk->x()-600,0,scene->width(), scene->height());
    if(con2 < 20 && b2 == 0)
    {
...
void Form::moverjefe()
{
    ...
    if(jefe == 4)
    {
        ...
        if(con5 < 20 && b2 == 0)
        {
            ...
            for(int i = 0; i < r.length(); i++)
            {
                if(Jefe->collidesWithItem(r.at(i))) con2 = 22;
            }
            con5 ++;
        }
        else if(Jefe->collidingItems().empty() || b2 == 1) Jefe->setPos(Jefe->x(), Jefe->y()+8);
        else
        {
            if(con5 > 100) con5 = 20;
            con5++;
